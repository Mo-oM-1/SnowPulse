...
-- ============================================
-- 6. BETA vs S&P 500 (REGR_SLOPE)
-- Rolling 60-day beta using Marketplace SPY data
-- ============================================
CREATE OR REPLACE DYNAMIC TABLE GOLD.TICKER_BETA
    TARGET_LAG = '1 day'
    WAREHOUSE = SNOWPULSE_WH
    COMMENT = 'Rolling 60-day Beta vs S&P 500 (SPY) using REGR_SLOPE'
AS
...
SELECT
    TICKER,
    TRADE_DATE,
    ROUND(REGR_SLOPE(STOCK_RETURN, SPY_RETURN) OVER (
        PARTITION BY TICKER ORDER BY TRADE_DATE ROWS BETWEEN 59 PRECEDING AND CURRENT ROW
    ), 4) AS BETA_60D,
    ROUND(REGR_R2(STOCK_RETURN, SPY_RETURN) OVER (
        PARTITION BY TICKER ORDER BY TRADE_DATE ROWS BETWEEN 59 PRECEDING AND CURRENT ROW
    ), 4) AS R_SQUARED_60D,
    CASE
        WHEN REGR_SLOPE(STOCK_RETURN, SPY_RETURN) OVER (
            PARTITION BY TICKER ORDER BY TRADE_DATE ROWS BETWEEN 59 PRECEDING AND CURRENT ROW
        ) > 1.2 THEN 'HIGH_RISK'
        WHEN REGR_SLOPE(STOCK_RETURN, SPY_RETURN) OVER (
            PARTITION BY TICKER ORDER BY TRADE_DATE ROWS BETWEEN 59 PRECEDING AND CURRENT ROW
